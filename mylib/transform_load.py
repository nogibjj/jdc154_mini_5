...
def load(dataset="data/nfl-wide-receivers.csv"):
    ...
    c.execute(
        "CREATE TABLE nflReceivers (id INTEGER PRIMARY KEY AUTOINCREMENT, pfr_player_id TEXT,player_name TEXT,career_try FLOAT,career_ranypa FLOAT,career_wowy FLOAT,bcs_rating FLOAT)"
    )
    # insert
    c.executemany(
        "INSERT INTO nflReceivers (pfr_player_id,player_name,career_try,career_ranypa,career_wowy,bcs_rating) VALUES (?,?, ?, ?, ?, ?)",
        payload,
    )
    conn.commit()
    conn.close()
